...
class RunningMeanStd(nn.Module):
    def __init__(self, insize, epsilon=1e-05, per_channel=False, norm_only=False):
        ...
        self.register_buffer("running_mean", torch.zeros(in_size, dtype = torch.float))
        self.register_buffer("running_var", torch.ones(in_size, dtype = torch.float))
        self.register_buffer("count", torch.ones((), dtype = torch.float))
