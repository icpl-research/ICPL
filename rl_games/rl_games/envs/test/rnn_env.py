...
class TestRNNEnv(gym.Env):
    def __init__(self,  **kwargs):
        ...
        if self.multi_obs_space:
            spaces = {
                'pos': gym.spaces.Box(low=0, high=1, shape=(2, ), dtype=np.float64),
                'info': gym.spaces.Box(low=0, high=1, shape=(4, ), dtype=np.float64),
            }
            self.observation_space = gym.spaces.Dict(spaces)
        else:
            self.observation_space = gym.spaces.Box(low=0, high=1, shape=(6, ), dtype=np.float64)
    ...
    def step(self, action):
        ...
        if self.use_central_value:
            ...
            if self.multi_obs_space:
                ...
                obses["state"] = state.astype(np.float64)
        else:
            obses = obs
        if self.multi_head_value:
            pass
        ...
        return obses, np.array(reward).astype(np.float64), done, info
