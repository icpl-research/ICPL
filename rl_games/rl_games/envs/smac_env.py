...
class SMACEnv(gym.Env):
    def __init__(self, name="3m",  **kwargs):
        ...
        self.observation_space = gym.spaces.Box(low=0, high=1, shape=(self.env_info['obs_shape']+one_hot_agents, ), dtype=np.float64)
        self.state_space = gym.spaces.Box(low=0, high=1, shape=(self.env_info['state_shape'], ), dtype=np.float64)
    ...
    def _preproc_state_obs(self, state, obs):
        ...
        if self.apply_agent_ids:
            ...
            obs = np.array(obs)
            all_ids = np.eye(num_agents, dtype=np.float64)
