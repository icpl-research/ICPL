...
def compute_humanoid_observations(obs_buf, root_states, targets, potentials, inv_start_rot, dof_pos, dof_vel,
    ...
    right_leg_pos = obs[:, 12]
    left_leg_pos = obs[:, 13]

    abdomen_y = actions[:, 0]
    abdomen_z = actions[:, 1]
    abdomen_x = actions[:, 2]
    right_hip_x = actions[:, 3]
    right_hip_z = actions[:, 4]
    right_hip_y = actions[:, 5]
    right_knee = actions[:, 6]
    right_ankle_x = actions[:, 7]
    right_ankle_y = actions[:, 8]
    left_hip_x = actions[:, 9]
    left_hip_z = actions[:, 10]
    left_hip_y = actions[:, 11]
    left_knee = actions[:, 12]
    left_ankle_x = actions[:, 13]
    left_ankle_y = actions[:, 14]
    right_shoulder1 = actions[:, 15]
    right_shoulder2 = actions[:, 16]
    right_elbow = actions[:, 17]
    left_shoulder1 = actions[:, 18]
    left_shoulder2 = actions[:, 19]
    left_elbow = actions[:, 20]
