...
class AntJump(VecTask):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        self.velocity = self.initial_root_states[:, 7:10].clone()
        self.ang_velocity = self.initial_root_states[:, 10:13].clone()

        self.torso_position = self.initial_root_states[:, 0:3].clone()
...
def compute_success(
    ...
    # type: (Tensor, Tensor, Tensor, Tensor, float, float, float, float, float, float, float, float) -> Tuple[Tensor, Tensor]
