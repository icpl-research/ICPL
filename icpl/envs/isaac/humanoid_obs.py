class Humanoid(VecTask):
    ...
    def compute_observations(self):
        ...
        self.obs_buf[:], self.potentials[:], self.prev_potentials[:], self.up_vec[:], self.heading_vec[:], self.velocity[:] = compute_humanoid_observations(
...
def compute_humanoid_observations(obs_buf, root_states, targets, potentials, inv_start_rot, dof_pos, dof_vel,
    ...
    # type: (Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, float, Tensor, Tensor, float, float, float, Tensor, Tensor) -> Tuple[Tensor, Tensor, Tensor, Tensor, Tensor, Tensor]
    ...
    right_leg_contact_force = self.vec_sensor_tensor[:, 0:3]
    left_leg_contact_force = self.vec_sensor_tensor[:, 6:9]
    ...
    return obs, potentials, prev_potentials_new, up_vec, heading_vec, velocity